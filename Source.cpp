...
double linear_solution(double t, double x)
{
    ...
    else if (x >= -t / 2. && x <= 1. / 4 - t / 2.) { return 4 * x; }
...
int calculate_linear_explicit_write_file(FILE* file, FILE* file_table, int Ntau, int Mh)
{
    ...
    for (int j = 1; j < Ntau + 1; j++)
    {
        ...
        for (int i = 1; i < Mh; i++)
        {
            ...
            if (Ntau == 10 && Mh >= 1000) { Vup[i] = V[i] + (nu / 100) * (V[i + 1] - V[i]); }
        }
...
int calculate_linear_explicit_write_file_local(FILE* file_table, int Ntau, int Mh, int K)
{
    ...
    for (int k = 0; k < K; k++)
    {
        ...
        {
            if (-1. + i * h <= 0) { V_tmp[i] = 0.; }
            else if (-1. + i * h <= 1. / 4) { V_tmp[i] = 4 * (-1. + i * h); }
            else V_tmp[i] = 1.;
        }
...
int calculate_linear_implicit_write_file(FILE* file, FILE* file_table, int Ntau, int Mh, double w)
{
    ...
    //for (int i = 0; i < Mh + 1; i++) { Vup[i] = -1 + i * h; }
    //for (int i = 0; i < Mh + 1; i++) { printf("%d & %3f \n", i, Vup[i]); }
    write_vector_file(file, Vup, Mh + 1);
    write_vector_file(file, V, Mh + 1);
...
int calculate_nonlinear_explicit_write_file(FILE* file, FILE* file_table, int Ntau, int Mh)
{
    ...
    for (int j = 1; j < Ntau + 1; j++)
    {
        ...
        for (int i = 1; i < Mh; i++)
        {
            Vup[i] = V[i] * (1 + 1 / 2 * (V[i] - V[i - 1]) + fmod(nu, 10) / 100);
        }
...
int main(void)
{
   ...
   // Таблица 1
    ...
    // Таблица 2
    calculate_nonlinear_implicit_write_file(File7, File8, 10, 2000, pow(10, -9), 1);
    calculate_nonlinear_implicit_write_file(File7, File8, 10, 20000, pow(10, -9),1);
    calculate_nonlinear_implicit_write_file(File7, File8, 5, 2000, pow(10, -9), 1);
    calculate_nonlinear_implicit_write_file(File7, File8, 10, 10000, pow(10, -9),1);
    calculate_nonlinear_implicit_write_file(File7, File8, 5, 20000, pow(10, -9),1);
    calculate_nonlinear_implicit_write_file(File7, File8, 5, 1000, pow(10, -9), 1);
